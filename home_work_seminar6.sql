...
BEGIN
DECLARE even_numb_string VARCHAR(10000) DEFAULT "";
DECLARE i INT DEFAULT 2;
DECLARE comma VARCHAR(2) DEFAULT "";

WHILE i<= max_number DO
	IF i % 2 = 0 THEN		
		SET even_numb_string = CONCAT(even_numb_string, comma,  i);
        SET comma = ", ";
    END IF;
	SET i = i + 1;
END WHILE;
SELECT even_numb_string;
END $$
DELIMITER ;

CALL get_even_numbers(19);
...
('12','f','1994-02-07','12','2015-08-04 16:34:50','Adriannaport'),
...
('29','f','1988-02-16','29','1988-02-09 12:47:21','Adriannaport'),
...
DROP PROCEDURE IF EXISTS get_5_friends;
DELIMITER %%
CREATE PROCEDURE get_5_friends
(
	user_condition INT
)
BEGIN
...
CREATE TEMPORARY TABLE `tmp_table`	
	SELECT
		user_tab.user_id as id
        , "same community" as friends_from
	FROM
		users_communities as user_tab
	WHERE 
		user_tab.community_id IN (SELECT tmp.community_id FROM users_communities as tmp WHERE tmp.user_id = user_condition)
			AND user_tab.user_id != user_condition
    ...
    SELECT
        ...
        , "same hometown"
	FROM
		profiles as user_tab
	WHERE 
		user_tab.hometown IN (SELECT tmp1.hometown FROM profiles as tmp1 WHERE tmp1.user_id = user_condition)
			AND user_tab.user_id != user_condition
	UNION
    
    SELECT
		user_tab.initiator_user_id
        , "same friends of friends"
	FROM
		friend_requests as user_tab
	WHERE 
		user_tab.target_user_id IN 
			(	SELECT 
					tmp2.user_id 
				FROM 
					(SELECT
						user_tab.initiator_user_id as user_id
					 FROM 
						friend_requests as user_tab
					 WHERE 
						user_tab.target_user_id = user_condition
							AND user_tab.status = "approved"
					 UNION
					 SELECT
						user_tab.target_user_id as user_id
					 FROM 
						friend_requests as user_tab
					 WHERE 
						user_tab.initiator_user_id = user_condition
							AND user_tab.status = "approved") as tmp2) 
        AND user_tab.status = "approved"
        AND user_tab.initiator_user_id != user_condition
	UNION
    
    SELECT
		user_tab.target_user_id
        , "same friends of friends"
	FROM
		friend_requests as user_tab
	WHERE 
		user_tab.initiator_user_id IN 
			(	SELECT 
					tmp3.user_id 
				FROM 
					(SELECT
					 ...
					 WHERE 
						user_tab.initiator_user_id = user_condition
							AND user_tab.status = "approved") as tmp3) 
        AND user_tab.status = "approved"
        AND user_tab.target_user_id != user_condition;

DROP temporary TABLE IF EXISTS users_with_row_number;
CREATE temporary TABLE users_with_row_number
SELECT 
	id AS id,
    friends_from AS friends_from,
    ROW_NUMBER() OVER() AS r_number
FROM 
	`tmp_table`;

SELECT MAX(r_NUMBER) INTO @max_number
FROM users_with_row_number;

DROP temporary TABLE IF EXISTS five_users;
CREATE temporary TABLE five_users
SELECT DISTINCT 
	ROUND(( @max_number) * rand()) + 1 AS rnd
FROM 
...
SELECT 
    ...
    tmp_t.friends_from
FROM
    users_with_row_number as tmp_t
		JOIN 
			users as usr
				ON tmp_t.id = usr.id
WHERE tmp_t.r_number IN (SELECT rnd FROM five_users);
   
END%%
DELIMITER ;

CALL get_5_friends(1);

-- SELECT hometown as hometown, count(hometown) as count_users from `profiles` group by hometown 
