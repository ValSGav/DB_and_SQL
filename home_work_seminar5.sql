DROP DATABASE IF EXISTS home_work_seminar5;
CREATE DATABASE home_work_seminar5;
USE home_work_seminar5;
...
CREATE VIEW cars_up_to__25_thousand
AS SELECT *
...
SELECT * FROM cars_up_to__25_thousand;
...
CREATE OR REPLACE VIEW cars_up_to__25_thousand
AS SELECT *	
 FROM
	cars
 WHERE
...
CREATE OR REPLACE VIEW cars_VAG_without_VW
AS SELECT *	
 FROM
	cars
 WHERE
	name = "Audi" or name = "Skoda";
    
SELECT * FROM cars_VAG_without_VW;
...
SELECT
	*,
   IFNULL(TIMEDIFF(LEAD(station_time) OVER w, station_time), "") AS time_to_next_station
FROM 
	train_schedule
WINDOW
	w AS (PARTITION BY train_id ORDER BY station_time);
...
SELECT
	CONCAT(usr.firstname, " ", usr.lastname) as user_full_name
FROM
	friend_requests as fr
		JOIN users as usr
			ON IF(fr.initiator_user_id = @my_user, fr.target_user_id = usr.id, fr.initiator_user_id = usr.id)
WHERE 
	(fr.initiator_user_id = @my_user or fr.target_user_id = @my_user)
...
FROM
	friends as fr
		JOIN users as usr
			ON fr.initiator_user_id = usr.id
WHERE fr.target_user_id = @my_user
UNION
SELECT
	CONCAT(usr.firstname, " ", usr.lastname) as user_full_name
FROM
	friends as fr
		JOIN users as usr
			ON fr.target_user_id = usr.id
WHERE fr.initiator_user_id = @my_user;

-- 	Создайте представление, в котором будут выводится все сообщения, в которых принимал
-- 	участие пользователь с id = 1.

SELECT
	CONCAT(usr_from.firstname, " ", usr_from.lastname) as user_from_full_name,
    CONCAT(usr_to.firstname, " ", usr_to.lastname) as user_to_full_name,
    m.body as 'message'
FROM 	
    (SELECT body, from_user_id, to_user_id FROM messages WHERE from_user_id = @my_user or to_user_id = @my_user) as m
		LEFT JOIN users as usr_from
			ON m.from_user_id = usr_from.id
		LEFT JOIN users as usr_to
			ON m.to_user_id = usr_to.id;	

-- 	Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)
SELECT
	m.filename as media,
    l.likes as likes,
    CONCAT(usr.firstname, " ", usr.lastname) as use_full_name
FROM
	media as m
		LEFT JOIN (SELECT media_id, COUNT(id) as likes FROM likes GROUP BY media_id) as l
			ON m.id = l.media_id
		JOIN users as usr
			ON m.user_id = usr.id
WHERE
...
CREATE OR REPLACE VIEW users_data_up_to_30_age
AS
SELECT
	usr.firstname,
    usr.lastname,
...
WHERE
	YEAR('2023-09-01') - YEAR(prof.birthday) <= 20;

SELECT * from users_data_up_to_30_age;
...
SELECT
	CONCAT(usr.firstname, " ", usr.lastname) AS user_full_name,
    msg.count_msg AS messages,
    DENSE_RANK() OVER (ORDER BY msg.count_msg DESC) AS `rank`
FROM
	(SELECT COUNT(id) as count_msg, from_user_id FROM messages GROUP BY from_user_id) AS msg
		JOIN users as usr
			ON msg.from_user_id = usr.id;
...
SELECT
	msg.*,
    IFNULL(DATEDIFF(LEAD(msg.created_at) OVER (ORDER BY msg.created_at), msg.created_at), "") as diff_time
FROM
	messages as msg;
