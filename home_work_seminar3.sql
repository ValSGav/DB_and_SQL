DROP DATABASE IF EXISTS seminar3_home_work;
CREATE DATABASE seminar3_home_work;
USE seminar3_home_work;
...
("Сидр", "Сидрово", "Рабочий", 		10,	 20000, 35),
("Антон", "Антонов", "Рабочий", 	8,	 19000, 28),
("Юрий", "Юрков", "Рабочий", 		5,	 15000, 25),
("Максим", "Максимов", "Рабочий", 	2,	 11000, 22),
("Юрий", "Галкин", "Рабочий", 		3,	 12000, 24),
("Людмила", "Маркина", "Убрщик", 	10,	 10000, 49);


-- Задание 1 Отсортируйте данные по полю заработная плата (salary) в порядке: убывания; 
SELECT * FROM staff ORDER BY salary DESC;

-- возрастания 
SELECT * FROM staff ORDER BY salary;

-- Задание 2 Выведите 5 максимальных заработных плат (saraly)
...
SELECT 
	COUNT(post) as count_worker 
FROM 
	staff 
WHERE 
	post = "Рабочий" AND age BETWEEN 24 AND 49;

-- 5. Найдите количество специальностей

SELECT 
	COUNT(post_table.count_post) 
FROM 
	(SELECT 
		post AS count_post 
	 FROM 
...
CREATE TABLE sub_table SELECT 
	users.id AS user_id,
	users.firstname,
	users.lastname,
	IF(media.filename LIKE '%.html' or media.filename LIKE "%.doc" or media.filename LIKE "%.docx" ,1,0) AS count_docs, 
...
SELECT 
	firstname, 
    lastname, 
    SUM(count_docs) AS count_of_docs 
FROM 
	sub_table 
GROUP BY 
	firstname, 
...
SELECT 
	firstname, 
    lastname, 
    name_type AS document_type,  
    SUM(count_likes) AS count_of_likes 
FROM 
	sub_table 
GROUP BY 
	firstname, 
