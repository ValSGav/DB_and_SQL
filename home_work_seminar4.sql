DROP DATABASE IF EXISTS home_work_seminar4;
CREATE DATABASE home_work_seminar4;
USE home_work_seminar4;
...
SELECT
	sh.shopname as 'Название магазина', 
    c.name as 'Имя кота'
FROM 
	`shops` as sh
		JOIN `cats` c
	...
	`shops` as sh
		LEFT JOIN (	SELECT 
					...
					FROM 
						`cats` cat 
					WHERE 
						cat.name = "Murzik" or  cat.name = "Zuza") c
		ON sh.id = c.shops_id
...
SELECT
	YEAR('2023-08-27') - YEAR(us.birthday) as age,
	COUNT(lk.id)
FROM
	likes lk
		JOIN media md
			JOIN `profiles` us
				ON md.user_id = us.user_id
			ON md.id = lk.media_id
GROUP BY 
	age
HAVING 
	age <= 12;
...
SELECT 
	IF(us.gender = 'm', 'Мужчины', 'Женщины') as 'Пол',
    COUNT(lk.id) as 'Количество лайков'
FROM
	likes lk
		JOIN `profiles` us
				ON lk.user_id = us.user_id
GROUP BY IF(us.gender = 'm', 'Мужчины', 'Женщины');
...
-- (по желанию)* Пусть задан некоторый пользователь(выбираете сами, к примеру, Reuben). Из всех друзей этого пользователя найдите человека, который больше всех написал ему сообщений.
...
FROM
	messages as msg
		...
		JOIN users as usr_to
			ON msg.to_user_id = usr_to.id
WHERE usr_to.firstname = 'Reuben'
GROUP BY
	usr.firstname,
    usr.lastname